...
# ============================================================================
# Данный код генерирует синтетические медицинские данные на основе
# механистических моделей. Предназначен только для образовательных целей
# и обучения ML-моделей. Не использовать для клинической диагностики!
# ============================================================================
...
class DataGenerator:
    ...
    def __init__(self, seed: int = 42, diabetes_threshold: float = 18.7):
        ...
        Args:
            ...
            diabetes_threshold: Порог для диабета (15-20)
                15 = ~37% заболеваемость
                17 = ~25% заболеваемость
                18 = ~18% заболеваемость
                18.7 = ~15% заболеваемость (рекомендуется)
        """
        ...
        self.BMR_MUSCLE_CORRECTION = 8.5  # ккал/кг/день

        self.TYPICAL_MUSCLE_RATIO = 0.3  # типичная мышечная масса = 30% веса

        # Диапазоны валидности биомаркеров (раздел 8.3 справки)
        self.BIOMARKER_RANGES = {
            ...
            "weight": (40, 200),
            "bmi": (16, 50),
            "non_hdl_mgdl": (50, 232),  # 232 = 6.0 ммоль/л
            "non_hdl_mmol": (2.0, 6.0),
            "muscle_mass_factor": (0, 20),
        }

        self.MUSCLE_PARAMS = {
            "strength_to_training": 150,  # MET-минут на одну силовую тренировку
            "protein_requirement": 1.6,  # г/кг для положительного баланса
            "protein_deficit": 0.8,  # г/кг для отрицательного баланса
            "sleep_deficit_hours": 6,
            "max_muscle": 20,
            "min_muscle": 0,
            "mm_to_weight_kg": 0.5,  # перевод единиц индекса мышечной массы в кг
        }
    ...
    def _clip_biomarker(self, values: np.ndarray, marker_type: str) -> np.ndarray:
        """Hard clipping для биомаркеров согласно разделу 8.3 справки"""
    ...
    def _calculate_cvd_risk(
        ...
        """Расчёт риска ССЗ (SCORE2-like) с non-HDL и пачко-годами"""
    ...
    def _calculate_diabetes_risk(
        ...
        """Расчёт риска диабета (FINDRISC-like) с доп. факторами
        Используется скорректированный ИМТ (bmi_corrected) – защитный эффект мышечной массы."""
        age = self._to_array(age)
        ...
        kbju_imbalance = self._to_array(kbju_imbalance)
        ...
        points += (kbju_imbalance > 20).astype(float) * 2
    ...
    def calculate_health_risks(
        ...
        """Расчет медицинских рисков для когорты"""
        ...
        final_kbju = biomarkers_df[f"kbju_imbalance_{last_year}"].values
        ...
        print(f"📊 Проверка баланса классов: {class_balance_check}")
    ...
    def calculate_metabolic_rate(
        ...
        """Формула Mifflin-St Jeor без коррекции на мышечную массу"""
        if sex == "M":
    ...
    def calculate_bmr_corrected(
        ...
        """
        Расчет BMR с коррекцией на мышечную массу (раздел 1.1 справки)
    ...
    def calculate_tdee(self, bmr: float, met_minutes_week: float) -> float:
        """
        Расчет суточной потребности в энергии (TDEE) по классификации ВОЗ.

        Пороги MET-минут скорректированы для соответствия уровням активности:
        - < 450 MET-min/нед ≈ сидячий/легкая активность (1-2 дня)
        - 450-900 MET-min/нед ≈ умеренная активность (3-5 дней)
        - 900-1500 MET-min/нед ≈ высокая активность (6-7 дней)
        - > 1500 MET-min/нед ≈ очень высокая активность (спортсмены)

        Args:
            bmr: Базальный метаболизм (ккал/день)
            met_minutes_week: MET-минуты в неделю
        ...
        if met_minutes_week < 450:  # ~150 мин × 3 дня или меньше
            activity_factor = 1.2  # Сидячий/легкая активность
        elif met_minutes_week < 900:  # ~150 мин × 6 дней или ~300 мин × 3 дня
            activity_factor = 1.375  # Умеренная активность
        elif met_minutes_week < 1500:  # ~300 мин × 5 дней или интенсивные тренировки
            activity_factor = 1.55  # Высокая активность
        else:
            activity_factor = 1.725  # Очень высокая активность (спортсмены)
    ...
    def _generate_stress_event(
        ...
        """
        Генерация случайных стрессовых событий.
    ...
    def generate_lifestyle_history(
        ...
        """
        Генерация истории образа жизни с OU-процессами, эволюцией веса и мышечной массы,
        а также сохранением всех параметров питания для каждого года.
        """
        ...
        # Генерируем индивидуальные базовые уровни для OU-процессов
        ...
        # Инициализируем массивы для хранения истинных значений
        ...
        # Генерируем стрессовые события (общие для всех)
        ...
        # Массивы для хранения истинных значений веса, мышц и параметров питания
        ...
        # Массивы для параметров питания
        ...
        kbju_imbalance_arr = np.zeros((n_people, years))
        ...
        # Генерируем параметры питания для первого года
        ...
        kbju_raw = np.sqrt(delta_p**2 + delta_f**2 + delta_c**2)
        kbju_imbalance_arr[:, 0] = np.minimum(kbju_raw * 10, 100)
        ...
        # Основной цикл по годам (начиная со второго)
        for t in range(1, years):
            age = age_start + t
            prev_weight = weight_true[:, t - 1]
            prev_muscle = muscle_true[:, t - 1]

            # BMR и TDEE на основе предыдущего веса (для расчёта калорий)
            ...
            kbju_raw = np.sqrt(delta_p**2 + delta_f**2 + delta_c**2)
            kbju_imbalance = np.minimum(kbju_raw * 10, 100)
            ...
            # Общий вес = жировая масса + мышечная масса + прочее (кости, органы)
            ...
            kbju_imbalance_arr[:, t] = kbju_imbalance
        ...
        for year in range(years):
            ...
            data[f"kbju_imbalance_{year}"] = kbju_imbalance_arr[:, year]
    ...
    def calculate_biomarkers(
        ...
        """
        Расчет биомаркеров на основе образа жизни.
        ...
        # Истинные значения биомаркеров (без шума)
        ...
        # Извлекаем истинные веса и мышцы
        ...
        for year in range(1, years):
            ...
            # Дисбаланс КБЖУ
            kbju_imb = lifestyle_df[f"kbju_imbalance_{year}"].values
            ...
            hdl_change = (
                ...
                0.3 * (strength_met / 450)  # допущение: 450 MET-мин ≈ 3 тренировки/нед
            ...
            # TC (генетика уже в начальном значении)
            tc_change = (
            ...
            # HbA1c
            # NOTE: KBJU_Imbalance is normalized 0-100 scale (section 2.3.2)
        ...
        # Добавляем шум и сохраняем в результирующий DataFrame
        for year in range(years):
            # Вес с шумом
            ...
            # Биомаркеры с шумом (уже с hard clipping в процессе эволюции)
            ...
            biomarkers_df[f"hdl_mgdl_{year}"] = self._clip_biomarker(hdl_noisy, "hdl")
            biomarkers_df[f"total_cholesterol_mgdl_{year}"] = self._clip_biomarker(
                tc_noisy, "tc"
            )
            biomarkers_df[f"sbp_mmhg_{year}"] = self._clip_biomarker(sbp_noisy, "sbp")
            ...
            # Добавляем колонки мышечной массы без суффикса _true (для обратной совместимости)
    ...
    def _generate_mar_missing_values(
        ...
        """
        Генерация MAR-пропусков (Missing At Random) согласно разделу 7.4.2 справки.
    ...
    def create_aggregated_dataset(
        ...
        """Создание агрегированного датасета с опциональными MAR-пропусками"""
        ...
        # TODO: Implement MAR missing value simulation (2-15% NaN) - раздел 7.4.2 справки
        missing_info = {}
        if apply_mar:
            ...
            if missing_info:
                print(f"MAR-пропуски сгенерированы:")
                for col, count in missing_info.items():
                    print(
                        f"   • {col}: {count} пропусков ({count / len(aggregated) * 100:.1f}%)"
                    )
    ...
    def save_for_kaggle(
        ...
        """
        Сохранение в формате для Kaggle.

        Примечание: MAR-пропуски теперь генерируются в create_aggregated_dataset().
        Этот метод сохраняет данные как есть, если apply_mar=False.
        """
        ...
        # Если MAR не был применен в пайплайне, можно применить здесь (обратная совместимость)
        if (
            apply_mar
            ...
            if missing_info:
                print(f"Пропуски сгенерированы (MAR) при сохранении:")
                for col, count in missing_info.items():
                    print(
                        f"   • {col}: {count} пропусков ({count / len(df_copy) * 100:.1f}%)"
                    )
        ...
        metadata = {
            ...
            "description": (
                f"Синтетический датасет для прогнозирования медицинских рисков. "
                f"Содержит {len(df_copy)} записей, сгенерирован {datetime.now().strftime('%Y-%m-%d')}. "
            ...
            "citation": "Данные сгенерированы для курсовой работы по теме 'Прогнозирование медицинских рисков'",
            "missing_values_info": {
                "keep_missing": True,
        ...
        print(f"{name} сохранен: {filepath}")
        print(f"Размер: {df_copy.shape[0]} строк × {df_copy.shape[1]} колонок")
...
def main():
    ...
    print("=" * 70)
    print("ЗАПУСК ГЕНЕРАТОРА МЕДИЦИНСКИХ ДАННЫХ")
    print("WARNING: NOT FOR CLINICAL USE - ТОЛЬКО ДЛЯ ОБРАЗОВАТЕЛЬНЫХ ЦЕЛЕЙ")
    print("=" * 70)
    ...
    print(f"\nПараметры генерации:")
    print(f"  • Количество людей: {N_PEOPLE}")
    print(f"  • Период наблюдения: {YEARS} лет")
    print(f"  • Random seed: {generator.seed}")
    print(
        f"  • Diabetes threshold: {generator.diabetes_threshold} (целевая заболеваемость ~15%)"
    )
    print(
        f"  • BMR коррекция: {generator.BMR_MUSCLE_CORRECTION} ккал/кг/день (раздел 1.1)"
    )
    print(f"  • Hard clipping: включено для всех биомаркеров (раздел 7.4.5)")

    print(f"\n1/5 Генерация базовой когорты...")
    cohort = generator.generate_cohort(N_PEOPLE)
    generator.save_for_kaggle(cohort, "01_cohort_baseline")

    print(f"\n2/5 Генерация истории образа жизни за {YEARS} лет...")
    lifestyle = generator.generate_lifestyle_history(cohort, years=YEARS)
    generator.save_for_kaggle(lifestyle, "02_lifestyle_history")

    print(f"\n3/5 Расчет биомаркеров с hard clipping...")
    biomarkers = generator.calculate_biomarkers(cohort, lifestyle, years=YEARS)
    generator.save_for_kaggle(biomarkers, "03_biomarkers_history")

    print(f"\n4/5 Расчет медицинских рисков...")
    risks = generator.calculate_health_risks(biomarkers, cohort, years=YEARS)
    generator.save_for_kaggle(risks, "04_health_risks")

    print(f"\n5/5 Создание агрегированного датасета с MAR-пропусками...")
    # MAR-пропуски генерируются внутри create_aggregated_dataset (раздел 7.4.2)
    ...
    generator.save_for_kaggle(
        aggregated, "05_aggregated_dataset_with_missing", apply_mar=False
    )

    print("\n" + "=" * 70)
    print("СТАТИСТИКА РАСПРЕДЕЛЕНИЯ РИСКОВ:")
    print("=" * 70)
    ...
    for disease, risk in risk_stats.items():
        print(f"  • {disease}: средний риск {risk:.2%}")
    ...
    print("НАПОМИНАНИЕ: Данные синтетические, НЕ для клинического использования")
    print("=" * 70)
